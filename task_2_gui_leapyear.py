...
class LeapYearGUI:
    def __init__(self):
        ...
        self.dash_between_date_entries = tk.Label(self.entry_frame, text="\N{EN DASH}", font=("Ariel", 20))
        self.date_entry_2 = tk.Entry(self.entry_frame, width=4, font=("Ariel", 20))
        self.submit_btn = tk.Button(self.entry_frame, text="Find leap years", font=("Ariel", 14),
        ...
        # TODO: sugaut rezultatą ir jį parodyt kaip label
        # result frame
        self.result_frame = tk.Frame(self.root)
        self.result_label = tk.Label(self.result_frame, text="Result will be displayed here", font=("Ariel", 14))
        # self.result_listbox = tk.Listbox(self.result_frame)
        # self.leap_year_list = self.find_leap()
    ...
    def validate(self):
        if len(self.date_entry1.get()) > 4 and len(self.date_entry_2.get()) > 4:
            return False
        elif not self.date_entry1.get().isdigit() and not self.date_entry_2.get().isdigit():
            return False
        return True
    ...
    def find_leap_and_show_result(self):
        ...
        self.result_listbox = tk.Listbox(self.result_frame, font=("Ariel", 14), height=10, width=12)
        ...
        # self.lbl_result = tk.Label(self.root, text=f"Results:", font=("Ariel", 16))
        # # self.lbl_result.config(text=f"{result}", width=250)
        # self.lbl_result.pack(padx=5, pady=5, side="left")
        # self.leap_result = tk.Text(self.root, )
